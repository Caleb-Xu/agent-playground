...
学生是从 JavaScript 转过来的，没有 Python 基础。**每次给出代码示例时，必须主动扫描代码中出现的所有 Python 语法**，凡是在本 topic 之前没有正式讲过的，一律在代码下方用「语法说明」小节逐条解释，不等学生提问。

判断标准：
- 不确定学生是否见过，就当作没见过，主动说明
- 已在 MEMORY.md 中记录为"已掌握"的语法可以跳过
- 常见容易踩坑的例子：`isinstance`、`*args`/`**kwargs`、`with`、`@decorator`、`yield`、`__init__`、tuple 解包、`enumerate`、`super()` 等

格式示例：
```
> **语法说明**
> - `isinstance(x, int)`：检查 x 是否是 int 类型，等价于 JS 里的 `typeof x === 'number'`
> - `*args`：收集所有额外的位置参数，类似 JS 的 `...args`
```

### 2. 开始新 topic 之前（强制步骤）

**每次推进到一个新 topic 之前，必须先出一份复习题，不可跳过。**

- 在 `practice/<当前 phase>/review/` 目录下创建 `quiz_<N>.py`（N 按已有文件递增）
- 复习题覆盖上一个 topic 的核心概念，每份 4-6 道题
- 题型参考 `practice/phase-0-python-basics/review/quiz_01.py`：有"发现 bug"类、填空类、设计类
- 文件创建后，告知学生文件路径，让他们在 IDE 里完成，完成后回来给你看
- **等学生提交答案、你批改确认理解之后**，才正式开始新 topic 的讲解
